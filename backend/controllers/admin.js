...
exports.getDashboard = catchAsync(async (req, res, next) => {
  ...
  const transformedRecentSubscriptions = recentSubscriptions.map((sub) => ({
    ...
    totalEmployees: sub.plans.reduce(
...
exports.getAllSubscriptions = catchAsync(async (req, res, next) => {
  ...
  if (req.query.planType) {
    query = query.where('planType').equals(req.query.planType);
  }

  if (req.query.autoRenew) {
    query = query.where('autoRenew').equals(req.query.autoRenew);
  }

  if (req.query.customerEmail) {
    query = query
      .where('customerDetails.email')
      .regex(new RegExp(req.query.customerEmail, 'i'));
  }
...
exports.getSubscription = catchAsync(async (req, res, next) => {
  ...
  const enhancedSubscription = {
    ...
    totalEmployees: subscription.plans.reduce(
      (sum, plan) => sum + plan.numberOfEmployees,
      0
    ),

...
const envSchema = Joi.object({
    ...
    ADMIN_URL: Joi.string().uri().default('http://localhost:4000'),
...
if (error) {
    console.error('❌ Environment configuration error: ', error.details.map(d => `- ${d.message}`).join('\n'));
    process.exit(1);
}
...
module.exports = {
    ...
    company: {
        ...
        tagline: process.env.COMPANY_TAGLINE || 'Professional Security & Compliance Services',
        ...
        supportEmail: process.env.COMPANY_SUPPORT_EMAIL || 'support@secureguard.com',
